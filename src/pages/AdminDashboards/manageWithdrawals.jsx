...
const ManageWithdrawals = () => {
  ...
  const fetchWithdrawals = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/withdrawals`, {
  ...
  const handleAction = async (id, action) => {
      try {
          setError('');
          setSuccess('');
          await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/withdrawals/${id}/${action}`, {}, {
              headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
          });
