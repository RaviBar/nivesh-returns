        ...
        <div className="p-4 border-b border-gray-700">
            <div className="flex justify-between items-center cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
                <div>
                    ...
                    <p className="text-sm text-gray-400">{activeSubsCount} Active Subscription(s) | Total Invested: ₹{totalInvestedInPlan}</p>
                </div>
                    ...
                    {subs.map((sub) => (
                        <div key={sub._id} className="mb-4 pb-2 border-b border-gray-800 last:border-b-0">
                            ...
                            <div className="space-y-2.5">
                                ...
                                <span className="text-[#666D80] text-sm">₹{sub.totalEarned}</span>
                                </div>
            ...
            {investmentData.subscriptions.length === 0 ? (
              <p className="text-gray-400">You haven't purchased any plans yet.</p>
                ...
                Object.entries(groupedSubscriptions).map(([planName, subs]) => (
                    <PlanGroup key={planName} planName={planName} subs={subs} />
                ))
            )}
          </div>
</div>
